...
class SubmarineBehaviour : RidingBehaviour<SubmarineSettings, SubmarineState> {
    ...
    private fun applyVelocityFromInput(velocity: Vec3, vehicle: PokemonEntity, driver: Player, state: SubmarineState): Vec3 {
    ...
    private fun applyJump(velocity: Vec3, vehicle: PokemonEntity, driver: Player, state: SubmarineState): Vec3 {
    ...
    private fun applyGravity(velocity: Vec3, vehicle: PokemonEntity, settings: SubmarineSettings, state: SubmarineState): Vec3 {
        if (state.jumpBuffer.get() == -1 && vehicle.isInWater || vehicle.isUnderWater) {
    ...
    private fun applyStrafeRotation(vehicle: PokemonEntity, driver: Player, state: SubmarineState) {
    ...
    private fun hasStamina(state: SubmarineState): Boolean {
        return state.stamina.get() != 0f
    }

    private fun consumeStamina(vehicle: PokemonEntity, state: SubmarineState, drain: Float) {
    ...
    override fun maxUpStep(settings: SubmarineSettings, state: SubmarineState, vehicle: PokemonEntity) = 0f
